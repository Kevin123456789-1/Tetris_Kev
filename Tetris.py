...
class Figure:
    ...
    Figures = [
        ...
        [[0, 4, 5, 6], [1, 2, 5, 9], [1, 5, 9, 8], [4, 5, 6, 10]],  # Rev L
    ...
    def __init__(self, x_coord, y_coord):
        ...
        self.color = colors[self.type + 1]  # normal: [self.type + 1]
        self.rotation = 0
...
pygame.init()
screen = pygame.display.set_mode((500, 790))  # 380 670
pygame.display.set_caption('Tetris by Kev', 'DHotel.jpg')
TetrisImg = pygame.image.load('Tetris.png')
pygame.display.set_icon(TetrisImg)
...
while not done:
    ...
    # Background
    background = pygame.image.load('Tetris BG 4.jpg')

    screen.fill(color = WHITE)  # color = WHITE
    # Background Img
    ...
    for i in range(game.height):
        for j in range(game.width):
            if game.field[i][j] == 0:
                ...
                just_border = 0
            pygame.draw.rect(screen, color, [107 + j * zoom, 35 + i * zoom, zoom, zoom], just_border)
            ...
            for j in range(4):
                ...
                if p in game.Figure.image():
                    pygame.draw.rect(screen, game.Figure.color,
                                     [107 + (j + game.Figure.x) * zoom, 35 + (i + game.Figure.y) * zoom, zoom, zoom])
